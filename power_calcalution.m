...
%% --- Parameters -----------------------------------------
Vs_pk    = max(abs(Vs));          % auto from data
f_line   = 50; f_sw = 100e3;
...
THD_i  = sqrt(max(Irms^2 - I1_rms^2, 0)) / max(I1_rms, 1e-9) * 100;
PF     = (I1_rms / max(Irms, 1e-9)) * DPF;
...
%% --- Helper: auto-pad ylim ------------------------------
function ylim_auto(ax, data, pad_frac)
    % Sets ylim with padding around actual data range
    if nargin < 3, pad_frac = 0.12; end
    dmin = min(data(:)); dmax = max(data(:));
    rng  = dmax - dmin;
    if rng < 1e-9, rng = 1; end
    pad  = rng * pad_frac;
    ylim(ax, [dmin - pad, dmax + pad]);
end

function xlim_auto(ax, tvec, pad_frac)
    if nargin < 3, pad_frac = 0.02; end
    tmin = min(tvec); tmax = max(tvec);
    rng  = tmax - tmin;
    pad  = rng * pad_frac;
    xlim(ax, [tmin - pad, tmax + pad]);
end

%% --- Color palette --------------------------------------
C.navy   = [0.02  0.27  0.49];
C.steel  = [0.24  0.47  0.71];
C.teal   = [0.06  0.43  0.32];
C.amber  = [0.39  0.31  0.04];
C.crimson= [0.63  0.11  0.11];
...
applyAx = @(ax, xl, yl) set(ax,...
    ...
    'TickDir','out',...
    'XLabel', xlabel(ax, xl, 'FontSize',FSl),...
...
%% =========================================================
%  PAGE 1  |  2×2 waveform plots — ALL axes auto-scaled
% =========================================================
...
hold(ax1,'off');
xlim_auto(ax1, t*1e3);
ylim_auto(ax1, [Vout; Vout_ref]);           % auto from actual data
legend(ax1,{'V_{out}','Ref 400 V'},'FontSize',FSl,'Location','best');
title(ax1,'① DC Output Voltage','FontSize',FSt,'FontWeight','bold','Color','k');
applyAx(ax1,'Time  (ms)','V_{out}  (V)'); grid(ax1,'on');
text(ax1,0.03,0.06,sprintf('Mean = %.2f V   Reg. err = %.3f %%',Vmean,Vereg),...
...
hold(ax2,'off');
xlim_auto(ax2, t*1e3);
ylim_auto(ax2, [iL; iref]);                 % auto from actual data
...
text(ax2,0.03,0.06,sprintf('Kp=%.1f  Ki=%.0f  Kd=%.1f',Kpc,Kic,Kdc),...
...
hold(ax4,'off');
xlim_auto(ax4, t_ss*1e3);
ylim_auto(ax4, [Vout_ss; Vout_ref; Vmean]);  % auto from actual data
...
text(ax4,0.03,0.06,sprintf('C=%g µF   R=%g Ω   f_{sw}=%g kHz',C_out*1e6,R_load,f_sw/1e3),...
    'Units','normalized','FontSize',8,'Color',C.navy,...
    'BackgroundColor',[0.92 0.95 1.00],'Margin',3);
...
xlim(ax5,[0 1000]);                          % frequency axis fixed 0-1kHz
ylim_auto(ax5, I_mag);                       % amplitude auto
...
hold(ax6,'off');
xlim_auto(ax6, t*1e3);
ylim_auto(ax6, Verror);                      % auto from actual error range
title(ax6,'⑥ Voltage Loop Error  (V_{ref} − V_{out})',...
    'FontSize',FSt,'FontWeight','bold','Color','k');
applyAx(ax6,'Time  (ms)','Error  (V)'); grid(ax6,'on');
text(ax6,0.03,0.06,sprintf('Kp=%.2f  Ki=%.1f  Kd=%.1f  N=%g',Kpv,Kiv,Kdv,Nv),...
...
hold(ax7,'off');
xlim_auto(ax7, t*1e3);
ylim_auto(ax7, Ierror);                      % auto from actual error range
title(ax7,'⑦ Current Loop Error  (i_{ref} − i_L)',...
    'FontSize',FSt,'FontWeight','bold','Color','k');
applyAx(ax7,'Time  (ms)','Error  (A)'); grid(ax7,'on');
text(ax7,0.03,0.06,sprintf('Kp=%.1f  Ki=%.0f  Kd=%.1f  N=%g',Kpc,Kic,Kdc,Nc),...
    'Units','normalized','FontSize',8,'Color',C.amber,...
    'BackgroundColor',[1.00 0.97 0.89],'Margin',3);
...
hold(ax8,'off');
xlim_auto(ax8, t*1e3);
ylim_auto(ax8, [Vpid; Ipid]);               % auto from actual duty cycle range
...
% auto y: show all bars + labels with padding
ylim_auto(ax9, [mv, 0, 1], 0.20);
grid(ax9,'on');
for k = 1:4
    text(ax9, k, mv(k) + 0.03*max(abs(mv)), sprintf('%.4f', mv(k)),...
        'HorizontalAlignment','center','FontSize',8,'FontWeight','bold','Color',mc(k,:));
end
if THD_i < 8
    text(ax9,0.5,0.04,'IEC 61000-3-2:  PASS (THD < 8%)',...
        'Units','normalized','FontSize',8,'FontWeight','bold',...
        'Color',[0.05 0.40 0.15],'BackgroundColor',[0.88 1.00 0.88],'Margin',3);
else
    text(ax9,0.5,0.04,'IEC 61000-3-2:  FAIL (THD >= 8%)',...
        'Units','normalized','FontSize',8,'FontWeight','bold',...
